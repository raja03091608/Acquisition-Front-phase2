...
@Component({
  ...
  templateUrl: './master-status.html',
})
export class MasterStatus implements OnInit {

  url = Apiendpoints.MASTER_STATUS;

  columnDefs: ColDef[] = [];

  reloadFlag = false;

  createUrl = Apiendpoints.MASTER_STATUS_CREATE;
  ...
  handleDeleteConfirm() {
    ...
    this.apiService
      .post(`${Apiendpoints.MASTER_STATUS_CREATE}`,{id:this.deleteId,status:3})
      .subscribe({
        next: () => {
