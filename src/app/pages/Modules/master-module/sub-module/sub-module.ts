...
@Component({
  ...
  templateUrl: './sub-module.html',
})
export class SubModule implements OnInit {

    url = Apiendpoints.MASTER_SUB_MODULE

  reloadFlag = false;
  createUrl =Apiendpoints.MASTER_SUB_MODULE+"/details"
...
ngOnInit() {
  ...
  apiUrl:'master/module?status=1'
},
    {
      ...
      label: 'Tentative Completion Moderate',
      type: 'text',
      required: true,
    },
    {
      key: 'targetted_month_complex',
      label: 'Tentative Completion Complex',
      type: 'text',
      required: true,
    }
  ];
...
setColumns() {
    this.columnDefs = [
      { headerName: 'Module Name', field: 'module.name' },
      { headerName: 'Sub Module Name', field: 'name' },
...
handleDeleteConfirm() {
  ...
  this.apiService
    .post(`${Apiendpoints.MASTER_SUB_MODULE_DETEAILS}`,{id:this.deleteId,status:3})
    .subscribe({
      next: () => {
